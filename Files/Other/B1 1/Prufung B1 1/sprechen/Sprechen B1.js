...
var qstHead;
var groups;
var quizTitle="Sprechen  b1 deutsch pour tous et tous pour deutsch";
var printCpRg=1;
var filename = 'Sprechen B1';
...
function completeInitValuate() {
 ...
 if (valid[2]==0) {
   ...
   doValuate(2,null,null);
 }
 if (valid[3]==0) {
   allAnsReport[3] = new initValuate4();
   doValuate(3,null,null);
 }
 CountAnswers();
}
...
function initShortQuestion() {
 qstHead[0]=" Sprechen Sie über die Punkte unten, machen Sie Vorschläge und reagieren Sie auf die Vorschläge Ihres Gesprächspartners/Ihrer Gesprächspartnerin.Planen und entscheiden Sie gemeinsam, was Sie tun möchten.   Besuch im Krankenhaus und Hilfe planen-  Wann besuchen? (Tag, Uhrzeit?)  - Wie  hinkommen?  - Was mitnehmen?  - Wie kann man helfen?    (vom Krankenhaus abholen, einkaufen, ...)  -  . .   ";
 qstHead[1]=" Sie sollen Ihren Zuhörern ein aktuelles Thema präsentieren. Dazu finden Sie hier fünf Folien. Folgen Sie den Anweisungen links und schreiben Sie Ihre Notizen und Ideen rechts daneben.            Thema 2            ";
 qstHead[2]=" Nach der Präsentation Ihres Partners/Ihrer Partnerin:  a)    Geben Sie eine Rückmeldung zur Präsentation Ihres Partners/Ihrer Partnerin       (z. B. wie Ihnen die Präsentation gefallen hat, was für Sie neu oder besonders interessant war usw.).   b)    Stellen Sie auch eine Frage zur Präsentation Ihres Partners/Ihrer Partnerin.  ";
 qstHead[3]="A";
}
...
function Constructor() {
  valid = new init_array(0,0,0,0);
  nScore = new init_array(0,0,0,0);
  maxScore = new init_array(0,0,0,0);
  qstHead = new init_array("","","","");
  qstRem = new init_array("","","","");
  initShortQuestion();
  initQuestionRemark();
  groups = new init_array(0,0,0,0);
  pesi = new init_array(0,0,0,0);
  allAnsReport = new init_array(4);
  ordineDomande = new init_array(0,1,2,3);
  typeOfQuest = new init_array(3,3,3,3);
  isQuestCustom = new init_array(0,0,0,0);
  omitPoint = new init_array(0,0,0,0);
}
